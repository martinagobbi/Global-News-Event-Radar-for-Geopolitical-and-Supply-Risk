...
# Genera tutti i task
    tasks = []
    for item in generate_urls_last_n_days(n_days):
        file_type = item["file_type"]

        # events / events_translation -> events ; mentions / mentions_translation -> mentions.
        # The filename is taken from the URL itself so the ".translation." infix is
        # preserved (e.g. 20240315143000.translation.export.CSV.zip) and English
        # and Translingual files coexist in the same folder without collisions.
        subdir = "events" if file_type.startswith("events") else "mentions"
        dest = RAW_DATA_DIR / subdir / Path(item["url"]).name
