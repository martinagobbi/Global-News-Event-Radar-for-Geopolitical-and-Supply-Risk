...
#Cerca nella cartella state il file last_seen.json. Se esiste, lo legge e carica in memoria un dizionario Python che contiene l'ultimo URL che hai scaricato in passato. Se non esiste, restituisce un dizionario vuoto {}
def load_state() -> dict:
...
#Effettua una richiesta HTTP (requests.get) verso l'URL ufficiale di servizio di GDELT (masterfilelist.txt)
def fetch_master_file_list(session: requests.Session) -> str:
    response = session.get(MASTER_FILE_LIST_URL, timeout=30)
    response.raise_for_status()
    return response.text

#Prende questo testo infinito, lo scorre riga per riga partendo dall'alto verso il basso e isola l'ultimo URL in assoluto in fondo al file che corrisponde al pattern desiderato
...
#Controlla se un file è già stato processato
def already_processed(state: dict, dataset: str, file_url: str) -> bool:
    return state.get(dataset) == file_url

# LOGICA DI DOWNLOAD, ESTRAZIONE, VALIDAZIONE per evitare di inviare a Kafka file corrotti o incompleti
