...
## Beyond silver: `full_latency_probe.py`

This tool stops at "landed in silver" by design — it has no database or network
access. For the two legs past that (new data reaching gold, and a profile edit
reaching gold), see [`full_latency_probe.py`](full_latency_probe.py) in this
same folder — its own docstring is the full write-up (why it's a separate
script, how it detects each leg, the run command, and the output columns).
Short version: it runs continuously against the live pipeline, needs
`--network pipeline_network` in addition to the volume mount above, and writes
`slice_latency_report.csv` and `preference_update_report.csv` here as events
happen.
